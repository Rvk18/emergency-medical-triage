...
| **gateway-config** | AgentCore Gateway OAuth and config (populated by **setup_agentcore_gateway.py**, not Terraform) | `gateway_url`, `gateway_id`, `region`, `client_info` (client_id, client_secret, token_endpoint, scope), `target_name`, `lambda_arn`, optional `eka_target_name`, `eka_lambda_arn` |
...
```bash
python scripts/setup_agentcore_gateway.py --save-to-secrets-only
```
...
To get only the secret name: `python scripts/load_gateway_config.py --secret-name`
...
- **GATEWAY_CONFIG_SECRET_NAME** – full secret name for gateway OAuth (default from api_config or `{NAME_PREFIX}/gateway-config`)
